...
investigator/scout side varies. `reasoning: max` is sent as `max` to
`gpt-5.6-terra` and clamped to `high` for the GLM models (provider rule in
`openai_chat.rs`). Muse Spark runs on OpenCode Go (`openai-responses`,
